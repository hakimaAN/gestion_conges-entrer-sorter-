...
public class EmployeeModel {
    ...
    private boolean checkFileExists(File file){
        if(!file.exists()){
            throw new IllegalArgumentException("le fichier n'est pas existe:"+file.getPath());
        }
        return true;}
    private boolean checkIsFile(File file){
        if (!file.isFile()){
            throw new IllegalArgumentException("le chemin specifiee ne pas un fichier"+file.getPath());
        }
        return true;
    }
